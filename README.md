...
## 相对基础三角形版本的改进（1/2/3/4/6/7）

为了便于课程对比，这里把关键改动整合如下：

1. **渲染对象升级（对应 1）**：从 3 顶点三角形升级为 8 顶点 + 12 边的立方体线框。
2. **旋转维度升级（对应 2）**：从单轴旋转升级为绕 X、Y 双轴旋转。
3. **视口映射优化（对应 3）**：使用窗口中心 + 固定视口像素尺寸，窗口缩放时主体保持稳定居中。
4. **线段绘制策略升级（对应 4）**：将每条边离散成多个采样点，再用 `canvas.circles` 模拟连续线段绘制。
6. **交互升级（对应 6）**：支持 `W/S/A/D` 与方向键控制，旋转更直观。
7. **窗口与兼容性升级（对应 7）**：使用 `ti.ui.Window` + `canvas`，并兼容检测 `resizable` 参数。

> 注：以上编号与评审对比项保持一致，因此保留了 1、2、3、4、6、7 的序号。
